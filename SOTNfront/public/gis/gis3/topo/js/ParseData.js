...
var ParseData = {
    ...
    init:function (box) {
        ...
        this.max_b=(document.getElementById('main').clientHeight-170)/2 - 30;
        this.max_a=document.getElementById('main').clientWidth/2-100;
    ...
    parseData: function (json,box) {
        ...
        aroom.setCenterLocation(this.center_x-aroom.getWidth()*1.1,this.start_y);
        this.box.add(aroom);

        var zroom=new RoomObj();
        zroom.setClient("int_id",json.z_transequiproom_id);
        zroom.setClient("zh_label",json.z_transequiproom_value);
        zroom.setCenterLocation(this.center_x+zroom.getWidth()*1.1,this.start_y);
        this.box.add(zroom);
        ...
        atrans.setCenterLocation(this.center_x-aroom.getWidth()*1.1,this.start_y-10);
        this.box.add(atrans);
        ...
        ztrans.setCenterLocation(this.center_x+zroom.getWidth()*1.1,this.start_y-10);
        this.box.add(ztrans);
    ...
    parsePath:function (json) {
        ...
        if(routes){
            ...
            if(size==2){   //椭圆
                ...
                atrans.setCenterLocation(this.center_x-this.max_a,this.center_y);
                ztrans.setCenterLocation(this.center_x+this.max_a,this.center_y);
                ...
                virtual2.setCenterLocation(this.center_x,this.center_y-this.max_b);
                ...
                for(var i=0;i<size;i++) {
                    ...
                    for(var j=0;j<route.length;j++) {
                        ...
                        if (from == null) {
                            ...
                            from.setCenterLocation(this.center_x + this.max_a * (Math.sin(angle)), this.center_y + this.max_b * (Math.cos(angle)));
                            this.box.add(from);
                        }
                        if (seg.a_alarm) {
                        ...
                        if (to == null) {
                            ...
                            to.setCenterLocation(this.center_x + this.max_a * (Math.sin(angle)), this.center_y + this.max_b * (Math.cos(angle)));
                            this.box.add(to);
                        }
                        if (seg.z_alarm) {
                        ...
                        if(beginangle<angle){
                            for(var offset=beginangle;offset<angle;offset+=spanAngle){
                                var offsetx=this.center_x + this.max_a * (Math.sin(offset));
                                var offsety=this.center_y + this.max_b * (Math.cos(offset));
