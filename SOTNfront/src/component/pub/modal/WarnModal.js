...
const getOperateMenu = (alarm, props) => (
  <Menu>
    <Menu.Item>
      <Link
        to={{
          pathname: `/main/fault/${props.pathName}/alarm-detail`,
          state: { alarm }
        }}
      >
        告警详情
    ...
    <Menu.Item>
      <Link
        to={{
          pathname: `/main/fault/${props.pathName}/influence`,
          state: { alarm }
        }}
      >
        影响分析
...
export const makeWarnTableDefaultColumns = props => ([
  {
    title: '状态标识',
    render: (text, record) => (
      <div>
        ...
        <Tooltip title={record.standardFlag === 2 ? '已标准化' : '未标准化'}>
  ...
  },
  {
    title: '操作',
    render: (text, record) => (
      <Dropdown overlay={getOperateMenu(record, props)}>
...
export default class WarnModal extends Component {
  ...
  static defaultProps = {
    ...
    totalCount: 0,
    onPageChange: () => {},
    columns: makeWarnTableDefaultColumns
  };
