...
import ChartExampleHomeChina from '../../pub/legend/ChartExampleHomeChina';
...
class ClientDetail extends React.Component {
  ...
  selectRate = idx => () => {
    ...
    this.setState(
      ...
      () => {
        const { customerName, rate } = this.state;
  ...
  pageChange = (
    ...
    this.setState({ pageNo, pageSize }, () => {
      const { pageNo, pageSize, customerName } = this.state;
      this.getLineList(customerName, pageNo, pageSize, searchKey);
    });
  };
  // 专线列表
  getLineList = (customer, currentPage, pageSize, nameLike) => {
    getAxios(
      'api/leased_lines',
      {
  ...
  // 故障专线数量(customerName：客户名称,nameLike:专线名称 )
  getFaultLine = customer => {
    getAxios(
  ...
  getAlarmActivity = customer => {
    ...
    getAxios('api/alarms/stats/num/sheet_send_status', { customer }, data => {
      this.setState({ sendAlready: data.values });
    });
  };
  // 告警列表
  getAlarmList = customer => {
  ...
  getClientData = () => {
    ...
    this.getFaultLine(customerName);
    this.getAlarmActivity(customerName);
    this.getAlarmList(customerName);
  ...
  gotoLineDetail = lineInfo => e => {
    e.stopPropagation();
    // e.nativeEvent.stopImmediatePropagation();
    gotoPathWithState('/main/business/line-detail', { lineInfo });
  };
      ...
      },
      {
        ...
        render: (text, record) => (
          <div style={{ color: '#2C9CFA' }} className="detail-name">
            <span onClick={this.gotoLineDetail(record)}>
              详情
            </span>
          </div>
        ),
