...
class BslineDetail extends React.Component {
  ...
  selectRate = idx => () => {
    ...
    this.setState(
      ...
      () => {
        const { lineName, rate } = this.state;
  ...
  // 告警列表
  getAlarmList = leasedLine => {
    getAxios('api/alarms', { leasedLine }, data => {
      this.setState({
  ...
  getClientData = () => {
    ...
    this.getAlarmActivity(lineName);
    this.getAlarmList(lineName);
