...
class SupportContent extends Component {
  ...
  componentWillReceiveProps(next) {
    if (this.props.orderClass !== next.orderClass) {
      this.setState({orderClass: next.orderClass},this.getAllData);
    }
    if (
      ...
      //如果tab发生切换且切换到当前页时
      this.setState({orderClass: next.activeKey},this.getAllData);
    }
  }
  ...
  //得到工单按业务保障等级分类统计
  getLevelData = (province, timeRange, type) => {
    // 得到全部工单饼图数据和工单总数
    postAxios(
      'api/flows/stats/security_level',
      { province, timeRange, type },
      data => {
        this.setState({
    ...
    postAxios(
      'api/flows/stats/security_level',
      { chain: '在途', province, timeRange, type },
      data => {
        this.setState({
    ...
    postAxios(
      'api/flows/stats/security_level',
      { chain: '归档', province, timeRange, type },
      data => {
        this.setState({
  ...
  //得到工单分省统计
  getProvinceData = (radioValue, province, timeRange, type) => {
    if (radioValue === '勘查及时率') {
      postAxios(
  ...
  onStartChange = (date, dateString) => this.setState({ startDate: date });

  onEndChange = (date, dateString) => this.setState({ endDate: date });

  handlePro = value => this.setState({province: value});
  ...
  search = () => {
    ...
    this.setState({timeRange: timeRange}, this.getAllData);
  };
      ...
      },
      {
        ...
        render: (text, record) => (
          <div className="control-line" title={record.finishtime}>
            {moment(parseInt(record.finishtime)).format('YYYY-MM-DD hh:mm:ss')}
          </div>
        ),
      ...
      },
      {
        ...
        render: (text, record) => (
          <div className="control-line" title={record.limitTime}>
            {moment(parseInt(record.limitTime)).format('YYYY-MM-DD hh:mm:ss')}
          </div>
        ),
              ...
              <Form onSubmit={this.handleSubmit}>
                <div className="left">
                  <Form.Item label="业务类型">
                    {getFieldDecorator('businessType', {
                      initialValue: ''
                    })(<Select>{businessTypeOptions}</Select>)}
                  </Form.Item>
                ...
                <div className="right">
                  <Form.Item>
                    ...
                    <Button className="reset-btn" onClick={this.onTableReset}>
                      重置
                    </Button>
                  </Form.Item>
                </div>
