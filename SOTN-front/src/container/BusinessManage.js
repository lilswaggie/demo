...
class BusinessManage extends Component {
  ...
  //     console.log(prevProps.business.type, this.props.business.type);
  //     return null;
  // }
  ...
  getTopData = (businessType = this.props.business.type) => {
    // 查询客户的分不同服务等级的统计
    getAxios('api/customers/stats/num/level', { businessType }, data => {
      this.setState({
        clientElement: {
          sum: data.summary,
    ...
    // 故障客户数统计
    getAxios('api/customers/stats/num/fault', { businessType }, data => {
      this.setState({
    ...
    // 客户的行业TopN统计
    getAxios('api/customers/stats/top_industry', { businessType }, data => {
      this.setState({
    ...
    // 专线的行业TopN统计
    getAxios('api/leased_lines/stats/top_industry', { businessType }, data => {
      this.setState({
  ...
  // 切换全部，国际和政务
  handleInter = (bool1, bool2, bool3) => {
    this.setState({
      business: [bool1, bool2, bool3]
    });
    if (!bool1 && bool2 && !bool3) {
      // 切换到国际时(重新请求接口数据，初步定义国际是1政企是2全部是0)
      this.setState({
        businessType: 1
      });
      this.getTopData(1);
    } else if (!bool1 && !bool2 && bool3) {
      // 切换到政企时(重新请求接口数据，初步定义国际是1政企是2全部是0)
      this.setState({
        businessType: 2
      });
      this.getTopData(2);
    } else {
      // 切换到全部时(重新请求接口数据，初步定义国际是1政企是2全部是0)
      this.setState({
        businessType: 0
      });
      this.getTopData();
      ...
      <div className="busimanage">
        <div className="title">
          ...
          <div className="item2" style={{ display: 'none' }}>
            <span
              className={this.state.business[0] ? 'active' : 'noneact'}
              onClick={() => this.handleInter(true, false, false)}
            >
              全部业务
            </span>
            <span
              className={this.state.business[1] ? 'active' : 'noneact'}
              onClick={() => this.handleInter(false, true, false)}
            >
              国际业务
            </span>
            <span
              className={this.state.business[2] ? 'active' : 'noneact'}
              onClick={() => this.handleInter(false, false, true)}
            >
              政企业务
            </span>
          </div>
